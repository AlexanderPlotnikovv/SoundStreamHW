...
class DecoderBlock(nn.Module):
    def __init__(self, N, S, length):
        ...
        self.net = nn.Sequential(
            nn.ELU(),
            nn.ConvTranspose1d(N, N // 2, kernel_size=2 * S, stride=S, padding=S // 2),
            ResidualUnit(N // 2, 1),
            ResidualUnit(N // 2, 3),
            ResidualUnit(N // 2, 9),
        )
...
class Decoder(nn.Module):
    def __init__(self, C=32, K=128, length=8000, strides=None):
        ...
        layers = [nn.Conv1d(K, (2 ** len(strides)) * C, kernel_size=7, padding=3)]
        ...
        layers += [nn.ELU(), nn.Conv1d(C, 1, kernel_size=7, padding=3), nn.Tanh()]
